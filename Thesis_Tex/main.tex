...
An anomaly will be defined as follows: an abnormal, rarely occurring data point or sequence, that has to be be detectable with exclusively context independent methods. Individual anomalous data points will be referred to as "point based" anomalies. Multiple consecutive anomalous points, each of which might be unremarkable on their own, while displaying unusual behavior as a sequence,  will be referred to as "sequence based" or "collective" anomalies \parencites[p.~3]{liu2024elephant}[p.~8]{chalapathy2019deeplearninganomalydetection}. Further more, there is the category of 'contextual anomalies', which are anomalies that deviate from an expected distribution, relying on a context that can be found entirely within the time series itself (eg. a window) \parencite[p.1699]{paparrizos2022tsb}. Context dependent anomalies that require additional external information about the field or time series will not be considered in this thesis. \\
...
\begin{itemize}
    \item \textit{Precision / Range Precision:} number of correctly identified anomalies over all identified anomalies.
    \item \textit{Recall (TPR) / Range Recall:} number of correctly identified anomalies over all actual anomalies.
...
\textbf{Numenta Anomaly Benchmark (NAB), 2015:} The Numenta Anomaly Benchmark is often considered to be the first large scale open source benchmarking environment for TSAD. At the time of release, it contained 58 datasets, made up of a mixture of artificial and real time series. The labels are first created by multiple humans, then combined into a ground truth by an algorithm. NAB uses a custom threshold dependent scoring function for the evaluation of a model's performance, designating high value to an algorithms ability to detect an anomaly as early as possible. It is designed specifically for real time anomaly detection, not static analysis. Therefore, only unsupervised models can be tested, with no training/test split of the data \parencites{Lavin_2015}.\par
...
\begin{itemize}
    \item \textit{Triviality:} Many anomalies "are so simple that solving them seems pointless or even absurd" \parencites[p.~2]{wu2021current}.
...
\textbf{GutenTAG (2022):} \cite{SchmidlEtAl2022Anomaly} implement and evaluate 71 different algorithms on 976 time series across a variety of fields, both univariate and multivariate. Furthermore, they introduced the GutenTAG synthetic dataset generator, allowing for the creation of time series with different lengths, variances, amplitudes, frequencies, and dimensions. \par
...
I have chosen the NAB for this analysis, as it is commonly cited by many to be one of the earliest attempts at creating a framework for TSAD benchmarking and includes a variety of different datasets. I will visually inspect all provided time series and select some examples representing the above mentioned flaws.
...
\begin{itemize}
    ...
    \item \textbf{NORMA} and \textbf{Series2Graph} are commercially licensed models. 
\end{itemize}
...
\textbf{Baseline Load:} All four datasets have the same baseline load for their default state. A script performs continuous matrix multiplications, randomly selecting a matrix of size 3000, 3500, 4500, or 5000 and performing 3 multiplications of the selected size. This is followed by a sleep period of 4 seconds, before another set of matrix multiplication resumes. Such a sequence of multiplications followed by a sleep time will be defined as a "baseline instance" going forward. Each dataset will first run for a predetermined amount of time without any anomalies, which can be used as the training split for semi-supervised models. In terms of dataset size, when compared with those provided by TSB-AD, my datasets are on the shorter end, though still significantly longer than some of the shortest provided datasets. To determine a reliable anomaly free training split, I decided to, once more, stay in line with TSB-AD. Most datasets have 500-2000 data points set as the training split. With that in mind, I settled on 1200 data points for datasets 1-3 and 2000 data points for the longer dataset 4.
...
\textbf{Dataset 1 - Large Matrix Injection:} For the first dataset, a particularly large matrix multiplication is performed as the anomaly. The generation starts with a large matrix multiplication to prepare the memory and avoid any changes in idle RAM utilization after the first anomaly injection. Afterwards, these preparation matrix multiplications are removed from the time series for a clear training split.
\begin{itemize}
...
\textbf{Dataset 2 - Sleep Time Injection:} For the second dataset, an additional sleep time is performed instead of a matrix multiplication. This results in a sequence consisting of: regular sleep time -> anomaly sleep time -> regular sleep time. Given that the anomaly does not require more memory than the baseload calculations, no preparation is required at the start of the dataset generation.
\begin{itemize}
    \item \textit{Baseload} -- Matrix Size: [3000, 3500, 4500, 5000] | Number of Multiplications: 3 | Sleep Time: 4 sec
    \item \textit{Runtime} -- Total: 90 min | Anomaly-free: 20 min 
...
\textbf{Dataset 3 - Medium Matrix Injection:} For the third dataset, a matrix multiplication is performed as the anomaly. The size of the matrix falls between the already existing matrices in the baseload calculations. Given that the anomaly does not require more memory than the baseload calculations, no preparation is required at the start of the dataset generation.
\begin{itemize}
    \item \textit{Baseload} -- Matrix Size: [3000, 3500, 4500, 5000] | Number of Multiplications: 3 | Sleep Time: 4 sec
    \item \textit{Runtime} -- Total: 90 min | Anomaly-free: 20 min 
...
\textbf{Dataset 4 - Combined Anomaly Injection:} For the fourth dataset, all three anomalies are injected. The generation starts with a large matrix multiplication to prepare the memory and avoid any changes in idle RAM utilization after the first anomaly injection. Afterwards, these preparation matrix multiplications are removed from the time series for a clear training split. The same combined anomaly frequency is maintained as in the previous three datasets. The total length of the dataset is adjusted upwards. 
\begin{itemize}
    \item \textit{Baseload} -- Matrix Size: [3000, 3500, 4500, 5000] | Number of Multiplications: 3 | Sleep Time: 4 sec
    ...
    \item \textit{Combined Anomaly Frequency:} 1/50 baseload instances
\end{itemize}

With all three anomalies representing distinctly different scenarios that require unique detection methods, a time series that challenges a model to detect all of them punishes methods that are highly specialized for a specific anomaly type. \par
...
\subsection{Flaws in Numenta Anomaly Benchmark (NAB)} \label{nab_res}
...
Equally common are \textbf{mislabeled anomalies}. Examples can be found in Figure \ref{fig:mislab}. The upper dataset labels two points as anomalies, while completely ignoring the obvious peaks, which are unusual even when compared with the entirety of the dataset. The time series at the bottom only highlights one seemingly normal datapoint, while again ignoring the unusual peak directly to its right.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrrrrr}
        \toprule
        \textbf{Model} & \textbf{\makecell[r]{Avg. Abs.\\Diff.}} & \textbf{\makecell[r]{Max. Abs.\\Diff.}} & \textbf{\makecell[r]{File\\ID}} & \textbf{\makecell[r]{Num. of\\Anom.}} & \textbf{\makecell[r]{Datasets\\>0.05 Diff.}} & \textbf{\makecell[r]{Datasets\\>0.25 Diff.}} & \textbf{\makecell[r]{Datasets\\>0.50 Diff.}} \\
        \midrule
        ...
        \rowcolor{myLightGreen} MOMENT-ZS & 0.007 & 0.414 & 579 & 1 & 9 & 1 & 0 \\
        ...
        \rowcolor{lightgray} Sub-HBOS & 0.001 & 0.095 & 531 & 1 & 1 & 0 & 0 \\
...
\begin{table}[htbp]
    ...
    \noindent
    \caption{\label{tab:Table 4} Statistical summary of datasets 1-4}
\end{table}

Most datasets used for the TSB-AD benchmark are between 1000 and 50000 data points in length. Similarly, datasets 1-3 have around 5000 data points. This ensures a sufficiently large sample size, while keeping it small enough for me to manually go through the entire set and check for potential problems or mislabeled data points. Since dataset 4 contains 3 separate anomaly types and I intended to maintain a consistent anomaly ratio between all sets, I increased its size. Otherwise, when maintaining the same anomaly ratio, it would be possible for some anomaly type to not occur at all. \cite[pp.~8]{SchmidlEtAl2022Anomaly} define an acceptable anomaly ratio to be one below 10\%, as anything above it would be better considered to be a classification problem. The median anomaly ratio in the TSB-AD dataset is 4.85\% and \cite[p.~4]{wu2021current} consider the adequate number of anomalies to be exactly 1. The anomaly range of my datasets falls right between those definitions, at between 0.7\% and 2.2\%. The differences between datasets can be explained by the average length of an anomalous sequence depending on anomaly type. The actual anomaly ratio based on anomalous sequences to total sequences is kept steady between datasets, falling between 0.15\% and 0.22\%. Maintaining the sequence ratio ensures each dataset presents a consistent number of anomalous events to detect, which is more important for a fair comparison than the point ratio that can be skewed by the varying lengths of those events. \par
For the calculations of the difficulty measures, I used Relative Contrast (RC), as described by \cite[pp.~1704]{paparrizos2022tsb}. Since the authors did not specify their preferred Shape Based Distance (SBD) metric, I applied the commonly used Dynamic Time Warping (DTW), using the cdist\_dtw implementation provided by the tslearn library \parencites{DTW}. \cite[pp.~1704]{paparrizos2022tsb} do not clearly state how to decide on a sliding window for the comparison of individual sequences and only state to "split the time series by its period" \parencite[p.~1704]{paparrizos2022tsb}. I interpret the term "period" to mean a repeating pattern, as is common in time series analysis. With all 4 of my created datasets, a repeating period can be defined as a sequence of matrix multiplications followed by an idle state, which tends to be between 8 and 12 data points long. With that in mind, I selected the value 10 for my sliding window. Since I neither know the exact RC implementation of \cite{paparrizos2022tsb}, nor their used SBD metric, it is impossible to compare the here produced values with those provided in the TSB-UAD paper. Therefore, a comparison can only be made between my own datasets. A low RC score should indicate that it is difficult to effectively cluster the points in a dataset, making anomaly detection theoretically more challenging. Based on this assumption, dataset 4 should provide the easiest to detect anomalies. As will be shown later, this does not hold up.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrrrr}
        ...
        \hspace{1em}Avg. Score (St. Mod.) & 0.255 & 0.125 & 0.023 & 0.061 \\
        \hspace{1em}Avg. Score (NN Mod.) & 0.494 & 0.051 & 0.013 & 0.105 \\
        ...
        \hspace{1em}Avg. Score (St. Mod.) & 0.863 & 0.566 & 0.607 & 0.604 \\
        \hspace{1em}Avg. Score (NN Mod.) & 0.809 & 0.565 & 0.485 & 0.651 \\
...
\end{lstlisting}
\par
When applying this simple algorithm, it returns 110 True Positives, 0 False Positives, 0 False Negatives, and therefore a precision and sensitivity of 1 are achieved.\par
...
Furthermore, the TSB-AD benchmark replication was performed only once. With such a small sample size, the results may not be statistically significant and could be due to chance. These runs were also done on consumer hardware in a non-isolated environment, which might have affected the outcomes. Additionally, minor unrecognized differences in implementation or updates to the used third party packages and libraries could have caused some of the observed differences. \\
...
The TSB-AD benchmark replication was performed separately. Due to Github file upload limitations, the benchmarking datasets and evaluations were removed: \url{https://github.com/Penguinbeanie/Time-Series-Anomaly-Detection-Seminar}. A summary file, containing all results, is present in the first repository, titled 'merged\_all\_uni\_metrics.csv' and 'merged\_all\_multi\_metrics.csv'.
