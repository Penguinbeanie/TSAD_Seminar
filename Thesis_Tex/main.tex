...
\textbf{Dataset 1 - Large Matrix Injection:} For the first dataset, a particularly large matrix multiplication is performed as the anomaly. The generation starts with a large matrix multiplication to prepare the memory and avoid any changes in idle RAM utilization after the first anomaly injection. 
\begin{itemize}
...
This anomaly type simulates a process "hanging" or an unexpected period of system inactivity, which is a critical but non-peak anomaly often missed by simple tresholding.
...
A sizable portion of all anomalies found in the NAB dataset fit into the category of \textbf{trivial anomalies} and can be detected using simple tresholding functions. Examples of that can be found in Figure \ref{fig:trivial}. \par
...
The TSB-AD benchmark replication was performed separately. Due to Github file upload limitations, the benchmarking datasets and evaluations were removed: \url{https://github.com/Penguinbeanie/Time-Series-Anomaly-Detection-Seminar}. A summery file, containing all results, is present in the first repository, titled 'merged\_all\_uni\_metrics.csv' and 'merged\_all\_multi\_metrics.csv'.
